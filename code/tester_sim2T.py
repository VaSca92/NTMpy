...
if case == 1:
    ...
    s.absorption = [5e-9, 1e-8, 1e-8, 1e-8]
    sim = Sim2T()
    sim.setSource(s)
    
    sim.addLayer(length_Pt,[k_el_Pt,k_el_Pt],[C_el_Pt,C_lat_Pt],rho_Pt,[G_Pt],15)  
    sim.addLayer(100e-9,[k_el_Si,k_lat_Si],[C_el_Si,C_lat_Si],rho_Si,[G_Si],20)
    sim.addLayer(400e-9,[k_el_Si,k_lat_Si],[C_el_Si,C_lat_Si],rho_Si,[G_Si],20)
