...
class temperature(object): 
    ...
    def __init__(self): 
        self.plt_points     = 50                    #number of points in x grid
    ...
    def Msetup(self):
        #Deleting the first element of the default initialization
        ...
        x               = [np.zeros(num_points) for num_points in self.collocpts]
        x_plt           = np.zeros([np.size(length)-1,plt_points])
        knot_vector     = [np.zeros(num_points+order+1) for num_points in self.collocpts]
        basis           = np.zeros(np.size(length)-1)
        A0h = []; A1h = []; A2h = []; Ch = [];
        LayerMat        = []
        ...
        for i in range(np.size(length)-1):
            x[i]   = np.linspace(length[i], length[i+1] , self.collocpts[i])
            ...
            LayerMat.append(np.dot(A2hinter,np.linalg.inv(A0hinter)))
            A0h.append(A0hinter)
            A1h.append(A1hinter)
            A2h.append(A2hinter)
            Ch.append(Chinter)
        #Reshape the long string of appended Matrix, such that
        #rows: x-points; colums: i´th basis spline - Cancel
        #Ch => More points in x, but same number of basis splines
        #Clearing the interface points, to not double count 
        ##N               = num_of_points 
        plp             = plt_points
        interfaces      = len(self.collocpts)-1
        sizeA           = np.sum(self.collocpts)-interfaces
        ...
        doublec = np.cumsum(self.collocpts)[:-1]
        ...
        startA = 0; endA = 0
        startC = 0; endC = plp-1 
        for i in range(interfaces+1):
            startA = endA
            endA += self.collocpts[i]-1
            Abig[startA:endA,startA:endA+1]  = A0h[i][:-1,:]
            A1b[startA:endA+1,startA:endA+1] = A1h[i]
            A2b[startA:endA+1,startA:endA+1] = A2h[i]
            Cb[startC:endC+1,startA:endA+1]  = Ch[i] 
        ...
        if (all(self.conductivity) or all(self.heatCapacity) or all(self.init)) == False:
            ...
            #make the conductivity always a function # CHANGE HERE
        ...
        for i in range(interfaces): 
            end_i += self.collocpts[i] - 1
            intercondL.append(self.conductivity[i](initphi[end_i])*A1h[i][-1])
            intercondR.append(self.conductivity[i+1](initphi[end_i])*A1h[i+1][1])
        ...
        for i in range(interfaces):
            start_i = end_i
            end_i += self.collocpts[i] - 1
            Abig[end_i,start_i:end_i]    =  intercondL[i][:-1]#Lhs interface flow
            Abig[end_i,end_i+1:end_i+self.collocpts[i+1]]  = -intercondR[i][1:]#Rhs interface flow
            Abig[end_i,end_i]            = intercondL[i][-1] -intercondR[i][0]
    ...
    def addLayer(self,L,refind,conductivity,heatCapacity,rho, points):
        """
...
class simulation(object): 
    ...
    def addLayer(self,L,n,conductivity,heatCapacity,rho,coupling=0, points = False,*args):
        """
        Add parameters of every layer: 
        (length,conductivity[electron,lattice,spin],heatCapacity[electron, lattice, spin],density, coupling[E-L,L-S,S-E])
        The units in SI are: 
        ...
        """
        if not points:
            points = 12 # default number of collocation points
        ...
        if self.num_of_temp == 2:
            if (np.size(conductivity) or np.size(heatCapacity))<2: 
                ...
                return(128)
            self.temp_data_Lat.addLayer(L,n,conductivity[1],heatCapacity[1],rho, points)
        ...
        if self.num_of_temp == 3:
            if (np.size(conductivity) or np.size(heatCapacity) or np.size(coupling))<3: 
                ...
                return(128)
            self.temp_data_Lat.addLayer(L,n,conductivity[1],heatCapacity[1],rho, points)
            self.temp_data_Spin.addLayer(L,n,conductivity[2],heatCapacity[2],rho, points)
        ...
        self.temp_data.addLayer(L,n,conductivity[0],heatCapacity[0],rho, points)
            
        
    def relambdisize2(self, i):
        
        if isinstance(self.temp_data.conductivity[i], (int, float, type(np.array([])))):
            dummy8 = self.temp_data.conductivity[i]
            self.temp_data.conductivity[i] = lambda x, y: dummy8 + 0*x

        if isinstance(self.temp_data_Lat.conductivity[i], (int, float, type(np.array([])))):
            dummy9 = self.temp_data_Lat.conductivity[i]
            self.temp_data.conductivity[i] = lambda x, y: dummy9 + 0*x
        
        try:
            self.temp_data.conductivity[i](0,0)
        except:
            dummy6 = self.temp_data.conductivity[i]
            self.temp_data.conductivity[i] = lambda x, y: dummy6(x)
        try:
            self.temp_data_Lat.conductivity[i](0,0)
        except:
            dummy7 = self.temp_data_Lat.conductivity[i]
            self.temp_data_Lat.conductivity[i] = lambda x, y: dummy7(y)
        
        
    def interconditions(self,phi,interfaces,conductivity,A1h):
        """
        ...
        for i in range(interfaces):
            end_i += self.collocpts[i] - 1
            intercondiL.append(conductivity[i](phi[end_i])*A1h[i][-1]) 
            intercondiR.append(conductivity[i+1](phi[end_i])*A1h[i+1][1])
    ...
    def run(self):
        ...
        for i in range(len(self.collocpts)):
            self.relambdisize2(i)
        ...
        if self.temp_data_Spin:
            ...
            for i in tqdm(range(1,len(t)),position = 0):
                ...
                intercon_E = self.interconditions(phi_E[i-1],interfaces,self.temp_data.conductivity,A1h)    
                intercon_L = self.interconditions(phi_L[i-1],interfaces,self.temp_data_Lat.conductivity,A1h)
                intercon_S = self.interconditions(phi_S[i-1],interfaces,self.temp_data_Spin.conductivity,A1h)
                startf = 0; endf = 0
                #Construct all picewise flows and piecewise dphi. Iterate over layers
                for j in range(interfaces+1):
                    startf = endf
                    endf += self.collocpts[j] - 1
                ...
                #filling up Abig wiht the interface condition in the middle of the grid
                start_i = 0; end_i = 0
                for k in range(interfaces):
                    start_i = end_i
                    end_i += self.collocpts[k] - 1
                    #for the electron system 
                    Abig_E[end_i,start_i:end_i]    =  intercon_E[0][k][:-1]#Lhs interface flow
                    Abig_E[end_i,end_i+1:end_i+self.collocpts[k]]  = -intercon_E[1][k][1:]#Rhs interface flow
                    ...
                    Abig_L[end_i,end_i+1:end_i+self.collocpts[k]]  = -intercon_L[1][k][1:]#Rhs interface flow
                    Abig_L[end_i,end_i]            = intercon_L[0][k][-1] -intercon_L[1][k][0]
                    #for the Spin system
                    Abig_S[end_i,start_i:end_i]    =  intercon_S[0][k][:-1]#Lhs interface flow
                    Abig_S[end_i,end_i+1:end_i+self.collocpts[k]]  = -intercon_S[1][k][1:]#Rhs interface flow
    ...
    #=======End 3 temp Case =================================
        ...
        if self.temp_data_Lat: 
            ...
            for i in tqdm(range(1,len(t)),position = 0):
                ...
                #filling up Abig wiht the interface condition in the middle of the grid
                start_i = 0; end_i = 0  
                for k in range(interfaces): #Apply interface conditions for all layers in every time step 
                    #for the electron system 
                    start_i = end_i
                    end_i += self.collocpts[k] - 1
                    Abig_E[end_i,start_i:end_i]    =  intercon_E[0][k][:-1]#Lhs interface flow
                    Abig_E[end_i,end_i+1:end_i+self.collocpts[k+1]]  = -intercon_E[1][k][1:]#Rhs interface flow
                    Abig_E[end_i,end_i]            = intercon_E[0][k][-1] -intercon_E[1][k][0]
                    #for the lattice system
                    Abig_L[end_i,start_i:end_i]    =  intercon_L[0][k][:-1]#Lhs interface flow
                    Abig_L[end_i,end_i+1:end_i+self.collocpts[k+1]]  = -intercon_L[1][k][1:]#Rhs interface flow
                    Abig_L[end_i,end_i]            = intercon_L[0][k][-1] -intercon_L[1][k][0]
        ...
        #=============End 2Temp Case ========================
            ...
            for i in tqdm(range(1,len(t)),position = 0):
                ...
                intercon_E = self.interconditions(phi[i-1],interfaces,self.temp_data.conductivity,A1h) #get interface conditions for every time step
                startf = 0;endf = 0
                ...
                for k in range(interfaces): #Apply interface conditions for all layers in every time step 
                    ...
                    Abig[end_i,end_i+1:end_i+self.collocpts[k]]  = -intercon_E[1][k][1:]#Rhs interface flow
