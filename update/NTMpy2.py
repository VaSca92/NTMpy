...
class temperature(object): 
    ...
    def Msetup(self):
        ...
        x               = [np.zeros([np.size(length)-1, num_points]) for num_points in self.collocpts]
        x_plt           = np.zeros([np.size(length)-1,plt_points])
        knot_vector     = [np.zeros([np.size(length)-1, num_points+order+1]) for num_points in self.collocpts]
        basis           = np.zeros(np.size(length)-1)
        A0h = []; A1h = []; A2h = []; Ch = [];
        LayerMat        = [np.zeros((num_points,num_points)) for num_points in self.collocpts]
